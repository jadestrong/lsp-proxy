...
use crate::remote::connection::{Connection, ssh::SSHConnection};
use crate::remote::{RemoteServerConfig, RemoteAuth, RemoteMode};
...
pub fn load_remote_lsp_config() -> Result<RemoteLspConfig> {
    let config_path = std::env::var("LSP_PROXY_REMOTE_CONFIG")
        .unwrap_or_else(|_| {
...
impl RemoteLspConfig {
    ...
    pub fn from_legacy(hosts: HashMap<String, LegacyRemoteHostConfig>, host_mapping: Option<HashMap<String, String>>) -> Self {
        ...
        for (name, host_config) in hosts {
            let server_config = RemoteServerConfig {
                ...
                user: host_config.user.unwrap_or_else(|| std::env::var("USER").unwrap_or("root".to_string())),
                auth: match host_config.connection_type {
                    LegacyConnectionType::SshPipe => {
                        if let Some(ssh_config) = host_config.ssh_pipe {
                            RemoteAuth::SshConfig { host: Some(ssh_config.host) }
                        } else {
...
impl SshTunnel {
    pub async fn establish(config: &SshTunnelConfig) -> Result<Self> {
        info!("Establishing SSH tunnel to {}:{} via local port {}", 
        ...
        cmd.args(&[
            ...
            "-o", "StrictHostKeyChecking=no",  // 跳过主机密钥检查 (仅用于开发)
        ...
        match TokioTcpStream::connect(format!("127.0.0.1:{}", config.local_port)).await {
            Ok(_) => {
                info!("SSH tunnel established successfully on port {}", config.local_port);
            }
            Err(e) => {
...
/// SSH pipe connection using existing SSH infrastructure
pub struct SshPipeConnection {
    ssh_connection: Arc<SSHConnection>,
    server_binary: String,
    config_file: Option<String>,
    host: String,
    session_channel: Arc<Mutex<Option<russh::ChannelId>>>,
}

impl SshPipeConnection {
    pub async fn new(config: &RemoteServerConfig) -> Result<Self> {
        info!("Establishing SSH pipe connection to {}@{}", config.user, config.host);
        
        // Create SSH connection using existing infrastructure
        let ssh_connection = Arc::new(SSHConnection::new(config.clone()).await?);
        
        // Connect to the SSH server
        ssh_connection.connect().await?;
        
        // Run server installation script if needed
        ...
        let server_binary = match &config.mode {
            RemoteMode::Server { server_path: Some(path), .. } => {
        ...
        Ok(Self {
            ssh_connection,
            server_binary,
            config_file: None, // Can be extended later
            host: config.host.clone(),
            session_channel: Arc::new(Mutex::new(None)),
        })
    }
    
    async fn ensure_server_installed(ssh_connection: &SSHConnection, server_path: &Option<PathBuf>) -> Result<()> {
        ...
        if install_result.exit_code == 0 {
            ...
            Err(anyhow!("Server installation failed: {}", install_result.stderr))
        }
    }
    
    pub async fn send_message(&self, message: ProxyMessage) -> Result<Option<ProxyMessage>> {
        let msg_json = serde_json::to_string(&message)?;
        debug!("Sending to {}: {}", self.host, msg_json);
        
        // Build remote command
        let mut remote_cmd = format!("{} --stdio", self.server_binary);
        if let Some(config_file) = &self.config_file {
            remote_cmd.push_str(&format!(" --config {}", config_file));
        }
        
        // For now, use the simpler approach of executing the command each time
        // This can be optimized later to use persistent channels
        let result = self.ssh_connection
            .execute_command("bash", &["-c", &format!("echo '{}' | {}", msg_json, remote_cmd)])
            .await?;
        
        if result.exit_code != 0 {
            return Err(anyhow!("Remote command failed: {}", result.stderr));
        }
        
        // Parse response
        let response_content = result.stdout.trim();
        if response_content.is_empty() {
            return Ok(None);
        }
        
        debug!("Received from {}: {}", self.host, response_content);
        match serde_json::from_str::<ProxyMessage>(response_content) {
            Ok(response) => Ok(Some(response)),
            Err(e) => {
                warn!("Failed to parse response from {}: {} - {}", self.host, e, response_content);
                Ok(None)
            }
        }
    }
...
impl Drop for SshPipeConnection {
    fn drop(&mut self) {
        ...
        // SSH connection will be automatically cleaned up when dropped
    }
}
...
impl RemoteConnection {
    pub async fn new(host: &str, config: &RemoteServerConfig) -> Result<Self> {
        ...
        match connection_type {
            ConnectionType::Tcp => {
                info!("Establishing direct TCP connection to {}:{}", config.host, config.port.unwrap_or(9527));
    ...
    pub async fn send_message(&self, message: ProxyMessage) -> Result<Option<ProxyMessage>> {
        match self.connection_type {
            ConnectionType::Tcp | ConnectionType::SshTunnel => {
                self.send_tcp_message(message).await
            }
            ConnectionType::SshPipe => {
                self.send_pipe_message(message).await
            }
        }
    }
    ...
    async fn send_tcp_message(&self, message: ProxyMessage) -> Result<Option<ProxyMessage>> {
        ...
        let stream = self.tcp_stream.as_ref()
        ...
        // 读取响应 (如果期望有响应)
        if matches!(message, ProxyMessage::Ping | ProxyMessage::StartLspServer { .. } | 
                             ProxyMessage::StopLspServer { .. } | ProxyMessage::LspRequest { .. } |
            ...
            match buf_reader.read_line(&mut response_line).await {
                Ok(_) => {
                    ...
                    match serde_json::from_str::<ProxyMessage>(response_line.trim()) {
                        Ok(response) => Ok(Some(response)),
                        Err(e) => {
                            warn!("Failed to parse TCP response from {}: {} - {}", self.host, e, response_line);
                            Ok(None)
                        }
                    }
                }
    ...
    async fn send_pipe_message(&self, message: ProxyMessage) -> Result<Option<ProxyMessage>> {
        let ssh_pipe = self.ssh_pipe.as_ref()
    ...
    pub async fn start_lsp_server(&self, workspace_path: &Path, language: &str) -> Result<String> {
        ...
        match self.send_message(message).await? {
            Some(ProxyMessage::Response { id: _, result }) => {
                match result {
                    Ok(_) => {
                        let mut servers = self.remote_lsp_servers.write().await;
                        servers.insert(workspace_path.to_string_lossy().to_string(), server_id.clone());
                        info!("Started remote LSP server {} for {} on {}", server_id, language, self.host);
                        Ok(server_id)
                    }
    ...
    pub async fn forward_lsp_request(&self, server_id: &str, method: &str, params: Value) -> Result<Value> {
    ...
    pub async fn forward_lsp_notification(&self, server_id: &str, method: &str, params: Value) -> Result<()> {
...
impl RemoteLspManager {
        ...
        if let Ok(url) = Url::parse(uri) {
            if url.scheme() == "file" {
                ...
                if path.starts_with("/ssh:") || path.starts_with("/scp:") || 
                   path.starts_with("/tramp:") || (path.contains(":") && path.contains("@")) {
                    return true;
                }
            }
        }
    ...
    pub async fn get_connection(&self, host: &str) -> Result<Arc<RemoteConnection>> {
        ...
        let host_config = self.config.servers.get(&config_host)
            .ok_or_else(|| anyhow!("No configuration for host: {} (mapped from: {})", config_host, host))?;
        ...
        info!("Established connection to remote host: {} (config: {})", host, config_host);
        Ok(connection)
    }
    
    /// Start remote LSP server for a file
    pub async fn start_remote_lsp(&self, file_info: &RemoteFileInfo, language: &str) -> Result<String> {
        let connection = self.get_connection(&file_info.host).await?;
        let workspace_path = Path::new(&file_info.path).parent()
            .unwrap_or(Path::new("/"))
            .to_path_buf();
    ...
    pub async fn forward_request(&self, host: &str, server_id: &str, method: &str, params: Value) -> Result<Value> {
        let connection = self.get_connection(host).await?;
        connection.forward_lsp_request(server_id, method, params).await
    }
    
    /// Forward LSP notification to remote server
    pub async fn forward_notification(&self, host: &str, server_id: &str, method: &str, params: Value) -> Result<()> {
        let connection = self.get_connection(host).await?;
        connection.forward_lsp_notification(server_id, method, params).await
    }
}
